...
#define M_PId (3.1415926535897932384626433832795)
...
static void fft_step2(ShortComplex *x, unsigned int T, bool complement, const ShortComplex *Wstore)
{
...
void universal_fft(ShortComplex *x, int N, bool complement)
{
    ...
    if (1 << T == N)
    {
        ...
        fft_step2(x, T, complement, Wstore);
        if (complement)
        {
            for(int i= 0; i < N; i++)
                x[i]/= N;
